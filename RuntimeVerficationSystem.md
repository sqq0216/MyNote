...
|7|如何缩短Z3求解器的验证时间|是|　　
|8|如何对乱序事件进行验证|是|  
|9|netfilter模块中，不同的事件进入不同回调函数hook_func，那么多个事件是等待同一个完成量还是多个？|是|　    
|10|netfilter工作在数据链路层，在与上层通信时，skb线性化会对解包和打包有什么影响，或者说上层应用是否也存在类似skb非线性化的问题？|是|  
|11|在hook_func中使用了skb线性化，目的是为了将page_data和frag_list的数据补充到线性buffer中，如果不使用线性化会导致什么情况？|是|  
|12|实验环境配置选择桥接模式而非NAT模式是出于什么的考虑？|是|  
|13||是|　
